...
class RecommendationController3 extends Controller
{
     ...
     */
    public function data2DArray($dataType)
    {
        $xData = Movie::all();
    ...
    public function index()
    {
        ...
        //
        // Language cluster
        $LanguageData = $this->data2DArray('language');

        // Number of clusters
        $numberOfClusters = 10;

        // Kmean for Language
        $this->KmeansControl($numberOfClusters, $LanguageData, 'language');
        // End clock time in seconds 
        $end_time = microtime(true);
        // Calculate script execution time 
        $execution_time = ($end_time - $start_time);
        echo '<tab> Time Taken to Calculate ' . $execution_time . ' sec';
    }
    public function KmeansControl($numberOfClusters, $data, $type)
    {
        $numbers = 0;
        ...
        foreach ($clusters as $index => $cluster) {
            ...
            echo "Cluster $index: ($numberOfPoints) <br>";
            ...
            foreach ($cluster as $point) {
                ...
                foreach ($point as $key => $pointData) {
                    if ($key == 0) {
                        ...
                        if ($type == 'genre') {
                            $yData = Genre::find($pointData);
                        } elseif ($type == 'language') {
                            $yData = Language::find($pointData);
                        } elseif ($type == 'cast') {
                            $yData = Cast::find($pointData);
                        } elseif ($type == 'director') {
                            $yData = Director::find($pointData);
                        } elseif ($type == 'country') {
                            $yData =  Country::find($pointData);
                        } elseif ($type == 'pcompany') {
                            $yData = ProductionCompany::find($pointData);
                        }
                    } else {
                        if ($type == 'genre') {
                            $zData = Genre::find($pointData);
                        } elseif ($type == 'language') {
                            $zData = Language::find($pointData);
                        } elseif ($type == 'cast') {
                            $zData = Cast::find($pointData);
                        } elseif ($type == 'director') {
                            $zData = Director::find($pointData);
                        } elseif ($type == 'country') {
                            $zData =  Country::find($pointData);
                        } elseif ($type == 'pcompany') {
                            $zData = ProductionCompany::find($pointData);
                        }
                    }
                }
                echo "[" . $Movie->title . " ," . $yData->title . " ," . $zData->title . "]";
            }

            echo " <br>";
        }
        echo " <br> Total Data" . $numbers . ' ';
    }
